...
async function getLatestRelease(): Promise<GitHubFetchResult> {
  return fetchGitHubRelease(
    'https://api.github.com/repos/ReSukiSU-GKI/GKI_KernelSU_SUSFS/releases/latest',
    'GKI'
  );
}
...
// Ignore list functions (silently ignore messages from specific chats)
async function isChatIgnored(kv: KVNamespace, chatId: number): Promise<boolean> {
  const entry = await kv.get(`ignore:${chatId}`);
  return entry !== null;
}

async function addChatToIgnore(kv: KVNamespace, chatId: number, reason: string, addedBy: number): Promise<void> {
  await kv.put(`ignore:${chatId}`, JSON.stringify({ chatId, reason, addedBy, addedAt: new Date().toISOString() }));
}

async function removeChatFromIgnore(kv: KVNamespace, chatId: number): Promise<boolean> {
  const existed = await kv.get(`ignore:${chatId}`);
  if (!existed) return false;
  await kv.delete(`ignore:${chatId}`);
  return true;
}

async function listIgnoredChats(kv: KVNamespace): Promise<Array<{ chatId: number; reason: string; addedBy: number; addedAt: string }>> {
  const list = await kv.list({ prefix: 'ignore:' });
  const result: Array<{ chatId: number; reason: string; addedBy: number; addedAt: string }> = [];
  for (const key of list.keys) {
    const value = await kv.get(key.name);
    if (value) {
      try { result.push(JSON.parse(value)); } catch { /* skip */ }
    }
  }
  result.sort((a, b) => a.chatId - b.chatId);
  return result;
}
...
async function handleAdmin(
  ...
  switch (subCommand) {
    ...
    case 'ignorelist': {
      ...
      const ignored = await listIgnoredChats(kv);
      if (ignored.length === 0) {
        await sendMessage(botToken, chatId, '📋 当前没有任何被忽略的群组。', 'HTML', replyToMessageId, messageThreadId);
      } else {
        let msg = `<b>🚫 被忽略的群组 (${ignored.length})</b>\n\n`;
        ignored.forEach((c, i) => {
          msg += `${i + 1}. <code>${c.chatId}</code>\n`;
          msg += `   原因: ${c.reason}\n`;
          msg += `   操作者: ${c.addedBy} | 时间: ${c.addedAt}\n\n`;
        });
        await sendMessage(botToken, chatId, msg, 'HTML', replyToMessageId, messageThreadId);
      }
      break;
    ...
    case 'ignore': {
      ...
      if (parts.length < 2) {
        await sendMessage(botToken, chatId, '❌ 请提供群组 Chat ID。\nUsage: <code>/admin ignore &lt;chatId&gt; [原因]</code>', 'HTML', replyToMessageId, messageThreadId);
        break;
      }
      const targetChatId = parseInt(parts[1], 10);
      if (isNaN(targetChatId)) {
        await sendMessage(botToken, chatId, '❌ 无效的 Chat ID。', 'HTML', replyToMessageId, messageThreadId);
        break;
      }
      const reason = parts.slice(2).join(' ') || '未指定';
      const already = await isChatIgnored(kv, targetChatId);
      if (already) {
        await sendMessage(botToken, chatId, '❌ 该群组已在忽略列表中。', 'HTML', replyToMessageId, messageThreadId);
        break;
      }
      await addChatToIgnore(kv, targetChatId, reason, fromUserId);
      await sendMessage(botToken, chatId, `✅ 已忽略群组 <code>${targetChatId}</code>。\n机器人将不再响应该群组的任何消息。\n原因: ${reason}`, 'HTML', replyToMessageId, messageThreadId);
      break;
    }
    case 'unignore': {
      if (!kv) {
      ...
      if (parts.length < 2) {
        await sendMessage(botToken, chatId, '❌ 请提供群组 Chat ID。\nUsage: <code>/admin unignore &lt;chatId&gt;</code>', 'HTML', replyToMessageId, messageThreadId);
        break;
      }
      const targetChatId = parseInt(parts[1], 10);
      if (isNaN(targetChatId)) {
      ...
      const removed = await removeChatFromIgnore(kv, targetChatId);
      if (removed) {
        await sendMessage(botToken, chatId, `✅ 已取消忽略群组 <code>${targetChatId}</code>，机器人将恢复正常响应。`, 'HTML', replyToMessageId, messageThreadId);
      } else {
        await sendMessage(botToken, chatId, `❌ 该群组不在忽略列表中。`, 'HTML', replyToMessageId, messageThreadId);
      }
      break;
    }
    case 'cache': {
    ...
    if (request.method === 'POST') {
      try {
        ...
        if (update.message && update.message.text) {
          ...
          // Silently ignore messages from ignored chats (skip all commands)
          if (env.KV && chatType !== 'private') {
            const ignored = await isChatIgnored(env.KV, chatId);
            if (ignored) {
